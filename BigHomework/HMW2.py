# -*- coding: utf-8 -*-
"""
Created on Sat Jun 29 09:35:54 2019
...
#数据预处理
train_features = pd.read_csv('train_features.txt',delimiter='\t',header=None)
x_data=np.array(train_features)[:,1:]
test_features = pd.read_csv('test_features.txt',delimiter='\t',header=None)
x_test=np.array(test_features)[:,1:].T
...
def CorrSelection(x_data,y_data,x_val):
    ...
    for i in range(length):
        if all(x_data[:,i]==0):
            continue
        cor=np.corrcoef(x_data[:,i].T.astype('float'),y_data.astype('float'))
        Corr.append([i,abs(cor[0][1])])
    Corr.sort(key=takeOrder,reverse = True)
    Corr=np.array(Corr)
    featureList=Corr[0:100,0].astype('int')
    '''print("correlation coefficient feature:",featureList)#探究重要因子
    plt.figure(num=8, figsize=(8, 5)) 
    x1=range(len(featureList))
    plt.plot(x1,Corr[0:100,1],'*-')
    plt.xlabel('特征数')
    plt.ylabel('相关系数')
...
def filtering(x_train,y_train,x_val):
    ...
    x_val_new=x_val[:,label.T]
    #x_val_new=model.transform(x_val)
    #print("filtering select features:",label)
    return x_train_new,x_val_new
'''
#递归特征淘汰
...
def NeuralNetwork(x_train,y_train,x_val,y_val):
    ...
    history=model.fit(x_train, y_train_nn.astype('int'), validation_data = (x_val, y_val_nn.astype('int')), epochs=20, 
...
def RandomForest(x_train_new,y_train,x_val_new):
    ...
    grid_search.fit(x_train_new,y_train.astype('int'))
    ...
    dtree.fit(x_train_new,y_train.astype('int')) 
    pred=dtree.predict(x_val_new) 
    print(classification_report(y_val.astype('int'),pred))
...
filtering_SVM_list=[]
filtering_Logistic_Regression_list=[]
filtering_NeuralNetwork_list=[]
filtering_RandomForest_list=[]
...
for train_index, test_index in kf.split(data):
    ...
    filtering_SVM_list.append(SVM_Classify(x_train_new,y_train,x_val_new,y_val))
    filtering_Logistic_Regression_list.append(Logistic_Regression(x_train_new,y_train,x_val_new,y_val))
    filtering_NeuralNetwork_list.append(NeuralNetwork(x_train_new,y_train,x_val_new,y_val))
    filtering_RandomForest_list.append(RandomForest(x_train_new,y_train,x_val_new))
...
print("*************************************************************")
print("filtering_SVM_list:",np.mean(filtering_SVM_list),filtering_SVM_list)
print("filtering_Logistic_Regression_list:",np.mean(filtering_Logistic_Regression_list),filtering_Logistic_Regression_list)
print("filtering_NeuralNetwork_list:",np.mean(filtering_NeuralNetwork_list),filtering_NeuralNetwork_list)
#print("filtering_RandomForest_list:",np.mean(filtering_RandomForest_list),filtering_RandomForest_list)
...
for i in range(5):
   ...
   sgd = optimizers.SGD(lr=0.1, decay=1e-6, momentum=0.9, nesterov=True) 
   model.compile(optimizer= sgd, loss='categorical_crossentropy',metrics=['accuracy'])
   history=model.fit(x_train_new, y_train_nn.astype('int'), validation_data = (x_val_new, y_val_nn.astype('int')), epochs=20, 
          batch_size=128, callbacks=[best_model, early_stop]) 
   model.summary()
   Y_pre = np.argmax(model.predict(x_test_new),1)
   prediction_list.append(Y_pre)
...
for item in test_result:
    if item==0:
        ...
        y_test.append("mono")   
with codecs.open('problem22.txt','a')as f1:
    for item in y_test:
        f1.write(item+'\t')
...
x_data=data[:,:-1]
y_data=data[:,-1]
x_data_new,x_val_new = filtering(x_data,y_data,x_val)
